...
%%%%%%%%%%%%%   MY_ASC        %%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 lambda=0.000001;;tao=0.001;
lambda_m=0.000001;tao_m=0.001;
decimal=4;
[imgs_R imgs_G imgs_B]=load_images(glob(['Tra1_gnd_x' num2str(upscaling) '_' num2str(lambda*1000)], '*.bmp'));%load_imgs(path)
 
[imgs_Rt imgs_Gt imgs_Bt]=load_images(glob(['Tra1_my_x' num2str(upscaling) '_' num2str(lambda*1000)], '*.bmp')); 
%%%%%%%%%  rrrrrrrrrrrrrrrrrr   %%%%%%%%%%%%%%%%%%%%%%%%%
  
 mat_file = ['T2_conf_KmeansR_' num2str(dict_sizes(d)) '_finalx' num2str(upscaling) '_l' num2str( lambda_m*1000) '_t' num2str(tao*1000)];   

 if exist([mat_file '.mat'],'file')
        disp(['Load trained dictionary...' mat_file]);
        load(mat_file, 'T2_conf_KmeansR');
        ...
        toc(startt)       
        T2_conf_KmeansR=conf_KmeansR;
        save(mat_file, 'T2_conf_KmeansR');
        % train call       
 end
...
%%%%%%%%%  ggggggggggg  %%%%%%%%%%%%%%%%%%%%%%%%%
 mat_file = ['T2_conf_KmeansG_' num2str(dict_sizes(d)) '_finalx' num2str(upscaling) '_l' num2str( lambda_m*1000) '_t' num2str(tao*1000)];   

 if exist([mat_file '.mat'],'file')
        disp(['Load trained dictionary...' mat_file]);
        load(mat_file, 'T2_conf_KmeansG');
        ...
        toc(startt)       
        T2_conf_KmeansG=conf_KmeansG;
        save(mat_file, 'T2_conf_KmeansG');
        % train call        
 end
  
%%%%%%%%%  bbbbbbbbbbbbbbbbb   %%%%%%%%%%%%%%%%%%%%%%%%%
 mat_file = ['T2_conf_KmeansB_' num2str(dict_sizes(d)) '_finalx' num2str(upscaling) '_l' num2str( lambda_m*1000) '_t' num2str(tao*1000)];   

 if exist([mat_file '.mat'],'file')
        disp(['Load trained dictionary...' mat_file]);
        load(mat_file, 'T2_conf_KmeansB');
        ...
        toc(startt)   
        T2_conf_KmeansB=conf_KmeansB;
        save(mat_file, 'T2_conf_KmeansB');
        % train call        
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    fname = ['T2_MY_x' num2str(upscaling) '_K' num2str(dict_sizes(d)) '_max' num2str(nn_patch_max) '_d' num2str(num_centers_use) '_lambda' num2str( lambda_m*1000) '_t'  num2str(tao_m*1000) '.mat'];
    ...
    if exist(fname,'file')
       ...
       %load(fnameS);
        ...
        %%
       disp('Compute MY_T2 regressors');
       ttime = tic;
       tic
       [imgs_R imgs_G imgs_B]=load_images(glob(['Tra1_gnd_x' num2str(upscaling) '_' num2str(lambda*1000)], '*.bmp'));
       [imgs_Rt imgs_Gt imgs_Bt]=load_images(glob(['Tra1_my_x' num2str(upscaling) '_' num2str(lambda*1000)], '*.bmp')); 

       [plores_r phires_r] = collectSamplesScales_T(T2_conf_KmeansR,imgs_R,imgs_Rt, 12, 0.98);  
       [plores_g phires_g] = collectSamplesScales_T(T2_conf_KmeansG,imgs_G,imgs_Gt, 12, 0.98);  
       [plores_b phires_b] = collectSamplesScales_T(T2_conf_KmeansB,imgs_B,imgs_Bt, 12, 0.98);  
...
% l2 normalize LR patches, and scale the corresponding HR patches
  ...
  CluP2{i}= DH*pinv(DL'*DL+2*tao_m*DH'*DH-tao_m*P_sc'*DH'*P_Dic'*DH-...
  ...
  %CluP_S{i}= SH*pinv(DL'*DL+2*tao_m*SH'*SH-tao_m*P_sc'*SH'*P_Dic'*SH-...
    ...
    disp(['done : ' num2str(i) '/1024']);
        ...
        ttime = toc(ttime);        
        save(fname,'CluP2','ttime', 'number_samples');   
...
%}
    %% 
    %end
result_gnd = qmkdir(['Tra2_gnd_x' num2str(upscaling) '_' num2str(lambda_m*1000)]);
result_dirRGB = qmkdir(['Tra2_my_x' num2str(upscaling) '_' num2str(lambda_m*1000)]);
      %%
    %t = cputime;    
 ...
 comYUV_Tr2=[];  
    for i = 1:numel(conf.filenames)
        ...
        im = ycbcr2rgb(cat(3,img1{1},img2{1},img3{1}));
 
        [imgR, imgG, imgB] = load_images({f}); 
  ...
  %}
            ...
            Dic.MY= CluP1;
            startt = tic;
            res_T1= scaleup_MY_T1_m(Dic,T1_conf_KmeansR,T1_conf_KmeansG,T1_conf_KmeansB, lowR,lowG,lowB);
            lowRt{1}=res_T1{1}(:,:,1);
            lowGt{1}=res_T1{1}(:,:,2);
            lowBt{1}=res_T1{1}(:,:,3);
            Dic.MY = CluP2;
             res= scaleup_MY_sr(Dic,T2_conf_KmeansR,T2_conf_KmeansG,T2_conf_KmeansB, lowRt,lowGt,lowBt);
            
            toc(startt)
     %imgres=res{1};
     imgres = ycbcr2rgb(res{1});
     ...
     imwrite(result,[result_dirRGB  '/my'  img_dir(i).name]);  
      ...
      comYUV_Tr2=[comYUV_Tr2;sp_psnr; ];
     %comYUV_Tr3=[comYUV_Tr3;sp_psnr; sp_ssim; sp_rmse;];
        %conf.filenames{i} = f;
    end
    save resyuv_Tr2 comYUV_Tr2;
    end