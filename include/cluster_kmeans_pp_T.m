function [conf] = cluster_kmeans_pp_T(conf, hires,hirest, dictsize)
...
for i = 1:numel(patches) % Remove low frequencies
   % patches{i} = hires{i} - interpolated{i};
    patches{i} = hires{i} - hirest{i};
end
clear hires interpolated hirest
