function [lores hires] = collectSamplesScales_T(conf, ohires,ohirest, numscales, scalefactor)
%[plores phires] = collectSamplesScales(conf_Kmeans, load_images(glob('CVPR08-SR/Data/Training', '*.bmp')), 12, 0.98); 
...
for scale = 1:numscales
    ...
    chirest = resize(ohirest, sfactor, 'bicubic');
    chires = modcrop(chires, conf.scale); % crop a bit (to simplify scaling issues)
    chirest = modcrop(chirest, conf.scale);
    % Scale down images
    ...
    features = collect(conf, chirest, conf.upsample_factor, conf.filters);
    clear midres
    ...
    for i = 1:numel(patches) % Remove low frequencies
       % patches{i} = chires{i} - interpolated{i};
        patches{i} = chires{i} - chirest{i};
    end
    clear chires interpolated chirest
